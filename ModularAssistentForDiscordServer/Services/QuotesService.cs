...
public class QuotesService
{
    ...
    {
        await using MadsContext db = await _dbContextFactory.CreateDbContextAsync();
        List<QuoteDbEntity> entities = db.Quotes.Where(x => x.Id == id).ToList();
    ...
    {
        await using MadsContext db = await _dbContextFactory.CreateDbContextAsync();
        List<QuoteDbEntity> entities = db.Quotes.Where(x => x.QuotedUserId == id).ToList();
        db.Quotes.RemoveRange(entities);
        await db.SaveChangesAsync();
    }
    ...
    {
        await using MadsContext db = await _dbContextFactory.CreateDbContextAsync();
        List<QuoteDbEntity> entities = db.Quotes.Where(x => x.UserId == id).ToList();
        db.Quotes.RemoveRange(entities);
        await db.SaveChangesAsync();
    }

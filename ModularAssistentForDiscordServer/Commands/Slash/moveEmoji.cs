...
public class MoveEmoji : ApplicationCommandModule
{
    ...
    public async Task MoveEmojiAsync(InteractionContext ctx, [Option("Emoji", "Emoji which should be moved")] string pEmoji)
    {
        ...
        var selectResponse = await response.WaitForSelectAsync(ctx.Member, "moveEmojiChooseGuild-" + ctx.User.Id, TimeSpan.FromSeconds(60));
        ...
        await selectResponse.Result.Interaction.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent("Submitted").AsEphemeral());
